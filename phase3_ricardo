...
public class phase3 
{
   ...
   private static final String WINNER_STRING = "Win!";
   ...
   public static void main(String[] args)
   {
      ...
      compWinnings = new Card[NUM_CARDS_PER_HAND * 2]; // fix size and instantiate in main
      humanWinnings = new Card[NUM_CARDS_PER_HAND * 2]; //fix size instantiate in main
      ...
      //generates two random cards in the community 
      for (int i = 0; i < NUM_PLAYERS; i++)
      {
         ...
         if (i == 0)
         {
            playLabelText[i] = new JLabel("Opponent (" + computerWinningsCounter
               + ")", JLabel.CENTER);
         }
         else
         {
            playLabelText[i] = new JLabel("You (" + humanWinningsCounter
               + ")", JLabel.CENTER);
         }
   ...
   public static void playCard(int playerIndex, int cardIndex)
   {
      //play card and update GUI
      Card playedCard = LowCardGame.playCard(playerIndex, cardIndex);
      playedCardLabels[playerIndex].setIcon(GUICard.getIcon(playedCard));

      if (playerIndex > 0)
      {
         humanCard = playedCard;
         playCard(0,0);
      }
      else
      {
         System.out.println(playedCard);
         computerCard = playedCard;
      }
      gameOver();
   }

   //determine the winner and gives points per round 
   public static void roundOver()
   {
      char opponentCardVal = computerCard.getValue();
      char humanCardVal = humanCard.getValue();
      int opponentCardValRank = 0;
      int humanCardValRank = 0;

      //get value ranks to compare 
      for (int i = 0; i < Card.valuRanks.length; ++i)
      {
         if (Card.valuRanks[i] == opponentCardVal)
            opponentCardValRank = i;
         if (Card.valuRanks[i] == humanCardVal)
            humanCardValRank = i;
      }

      //determines the winner 
      if (opponentCardValRank > humanCardValRank)
      {
         //opponent is the winner 
         compWinnings[computerWinningsCounter++] = computerCard;
         compWinnings[computerWinningsCounter++] = humanCard;
         playLabelText[0].setText(WINNER_STRING + " (" + computerWinningsCounter
            + ")");

      }
      else if (opponentCardValRank < humanCardValRank)
      {
         //you win 
         humanWinnings[humanWinningsCounter++] = computerCard;
         humanWinnings[humanWinningsCounter++] = humanCard;
         playLabelText[1].setText(WINNER_STRING + " (" + humanWinningsCounter 
            + ")");
      }
      else 
      {
         //draw
         playLabelText[0].setText(DRAW_STRING + " (" + computerWinningsCounter + ")");
         playLabelText[1].setText(DRAW_STRING + " (" + humanWinningsCounter + ")");
      }
   ...
   {
      playLabelText[0].setText("Opponent (" + computerWinningsCounter + ")");
      playLabelText[1].setText("You (" + humanWinningsCounter + ")");
      ...
      for (int i = 0; i < LowCardGame.getHand(1).getNumCards(); i++)
      {
         // Player labels
         JButton tempLabel = new JButton(
            GUICard.getIcon(LowCardGame.getHand(1).inspectCard(i))
         );
         tempLabel.addActionListener(myCardTable);
         humanLabels[i] = tempLabel;
...
class CardTable extends JFrame implements ActionListener{
   ...
   public void actionPerformed(ActionEvent e)
   {
      ...
      {
         if (e.getSource() == phase3.humanLabels[i])
         {
            phase3.playCard(1, i);
            phase3.roundOver();
         }
